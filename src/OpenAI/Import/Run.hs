module OpenAI.Import.Run (runOne, planOne)
where
  ...
  case mbRowE of
    ...
    Right mbRow ->
      case planOne opts mbRow conv of
        Left report -> pure $ Right report
        Right (state, act) ->
          case opts.modeOpt of
            It.DryM -> pure $ Right $ dryReport mbRow conv state act
            It.WriteM -> runWrite mgr pool opts mbRow conv sourceKey state act


runWrite :: Maybe Ht.Manager -> Hp.Pool -> It.Opts -> Maybe Il.RowConv -> Jd.Conversation
    -> Text -> It.ConvState -> It.Action -> IO (Either Hp.UsageError Ire.Report)
runWrite mgr pool opts mbRow conv sourceKey state act =
  case act of
    It.AddFreshA -> do
      rawE <- Ir.addFresh pool conv
      finish mgr pool opts Nothing conv rawE
    It.UpdateKnownA -> case (mbRow, state) of
      (Just row, It.PresentCS {}) -> do
        rawE <- Ir.updateKnown pool row conv sourceKey
        finish mgr pool opts (Just row.uidConv) conv rawE
      (Just row, It.OlderCS {})
        | opts.policyOpt.allowOlderPol -> do
            rawE <- Ir.updateKnown pool row conv sourceKey
            finish mgr pool opts (Just row.uidConv) conv rawE
      _ -> pure $ Right $ failReport (uidFromState state <|> uidFromRow mbRow) conv "invalid update state"
    It.SkipOlderA -> pure $ Right $ skipReport (uidFromState state <|> uidFromRow mbRow) conv state
    It.SkipSameA -> pure $ Right $ sameReport (uidFromState state <|> uidFromRow mbRow) conv state
    It.FailA -> pure $ Right $ failReport (uidFromState state <|> uidFromRow mbRow) conv (failText state)


finish :: Maybe Ht.Manager -> Hp.Pool -> It.Opts -> Maybe Int64 -> Jd.Conversation
    -> Either Hp.UsageError (Either T.Text Ire.Report) -> IO (Either Hp.UsageError Ire.Report)
      ...
      case discE of
        ...
        Right (Right report1) -> do
          ...
          case sumE of
            ...
            Right (Right report2) -> pure $ Right report2

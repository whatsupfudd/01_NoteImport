...
selectAllConversationsDetailedRows :: Statement () (Vector (Int64, Text, Text, Double, Double))
...
-- TODO: manage the non-incremental input that didn't set either child_seq nor preorder_seq.
...
selectContents =
  [TH.vectorStatement|
    ...
    from oai.contents c
      ...
      join oai.nodes n on n.uid = m.node_fk
    where n.conversation_fk = $1::int8
    order by c.message_fk, c.seqnbr, c.uid
  |]
...
selectMultiModalParts =
  [TH.vectorStatement|
    ...
    from oai.multimodal_parts mp
      ...
      join oai.nodes n on n.uid = m.node_fk
    where n.conversation_fk = $1::int8
    order by mp.content_fk, mp.seqnbr, mp.uid
  |]

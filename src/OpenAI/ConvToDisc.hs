...
analyzeConversation conv =
  ...
  in
    case mbRoot of
      ...
      Just rootNode -> Right $ runFSM initCtxt mapping childMap rootNode.eidNd
...
handleUserMsg context msg =
  case firstTextContent msg.contentsMsg of
    Just partsV ->
      let
        userMsg = UserMessage { textUM = T.intercalate " |<part>| " (V.toList partsV), attachmentsUM = [] }
        timing = Timing { createTime = msg.createTimeMsg, updateTime = msg.updateTimeMsg }
      in
...
finalizeAssistant context msg mbFinalContent =
  let
    resp = maybe (ResponseAst {textRA = "No response"}) buildAssistantResponse mbFinalContent
  in
      ...
      let
        updMsg = case astMsg of
            AssistantMF timing prevMsg ->
              AssistantMF timing prevMsg { subActions = reverse prevMsg.subActions, response = Just resp }
            _ -> astMsg
      in
...
codeP context timing msg language responseFormatName text =
  let
    subAction = CodeSA Code {
          languageCC = language, responseFormatNameCC = responseFormatName, textCC = text
        }
    eiNewMsg = case context.currentMsg of
        Just prevMsg -> case prevMsg of
          AssistantMF t assistantMsg -> Right $ AssistantMF t assistantMsg { subActions = subAction : assistantMsg.subActions }
...
thoughtsP context timing msg _sourceAnalysisMsgId thoughts =
  let
    ...
    subActions = map (\aThought -> ReflectionSA Reflection { 
              summaryRF = aThought.summaryTh, contentRF = aThought.contentTh
            , chunksRF = decodeChunks aThought.chunksTh, finishedRF = Just aThought.finishedTh
            }
        ) (reverse thoughts)
    eiNewMsg = case context.currentMsg of
        Just prevMsg ->
          case prevMsg of
            AssistantMF t assistantMsg -> Right $ AssistantMF t assistantMsg { subActions = subActions <> assistantMsg.subActions }
            _ -> Left $ "assistant msg eid: " <> msg.eidMsg <> " is not an assistant message: " <> T.pack (show prevMsg)
        Nothing -> Right $ AssistantMF timing (AssistantMessage {
...
handleSystemMsg context msg =
  let
    systemMsg = SystemMessage { textSM = summarizeContents msg.contentsMsg }
    timing = Timing { createTime = msg.createTimeMsg, updateTime = msg.updateTimeMsg }
  in
...
handleToolMsg context msg =
  let
    toolMsg = ToolMessage { textTM = summarizeContents msg.contentsMsg }
    timing = Timing { createTime = msg.createTimeMsg, updateTime = msg.updateTimeMsg }
  in
...
respFromContent = \case
  Cv.CodeCT_Db _language _responseFormatName text -> ResponseAst { textRA = "CodeContent: " <> text }
  Cv.ExecutionOutputCT_Db text -> ResponseAst { textRA = "ExecutionOutputContent: " <> text }
  ...
  Cv.MultimodalTextCT_Db parts -> ResponseAst { textRA = "MultimodalTextContent: " <> T.pack (show parts) }
  Cv.ReasoningRecapCT_Db content -> ResponseAst { textRA = "ReasoningRecapContent: " <> content }
  Cv.SystemErrorCT_Db name text -> ResponseAst { textRA = "SystemErrorContent: " <> name <> " " <> text }
  Cv.TetherBrowsingDisplayCT_Db result summary assets tetherId -> ResponseAst {
        textRA = "TetherBrowsingDisplayContent: " <> result <> " " <> T.pack (show summary) <> " "
            <> T.pack (show assets) <> " " <> fromMaybeText "No tetherID" tetherId
      }
  Cv.TetherQuoteCT_Db url domain text title tetherId -> ResponseAst {
        textRA = "TetherQuoteContent: " <> url <> " " <> domain <> " " <> text <> " "
            <> title <> " " <> fromMaybeText "No tetherID" tetherId
      }
  Cv.TextCT_Db partsV -> ResponseAst { textRA = T.intercalate " |<part>| " (V.toList partsV) }
  Cv.ThoughtsCT_Db sourceAnalysisMsgId thoughts ->
    ResponseAst { textRA = showThoughts thoughts } -- "ThoughtsContent: " <> sourceAnalysisMsgId <> " " <>
  Cv.UnknownCT_Db contentType raw -> ResponseAst { textRA = "OtherContent: " <> contentType <> " " <> jsonValueToText raw }

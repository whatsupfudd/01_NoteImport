  ...
  let
    eidIssues =
      case Oid.eidMsgFromText message.idMsg of
        ...
        Right _ -> []
    {-
    createIssues = maybe [] (validateTime $ "message " <> quote message.idMsg <> " create_time") message.createTimeMsg
    updateIssues = maybe [] (validateTime $ "message " <> quote message.idMsg <> " update_time") message.updateTimeMsg
    -}

...
addMessageSession node_uid msg = do
  muid <- Tx.statement
    ...
    , msg.endTurnMsg, msg.weightMsg, Ae.toJSON (HM.fromList $ Map.toList msg.metadataMsg)
...
addAuthorSession msg_uid au = Tx.statement
    ...
    , Ae.toJSON (HM.fromList $ Map.toList au.metadataAu)) St.insertAuthorStmt
...
addNodeTreeSession :: Int64 -> Map.Map Text Node -> Text -> Maybe Int64 -> Tx.Transaction (Either String ())
addNodeTreeSession discussionID mapping nodeEid mbParentID =
  let
    mbNode =  Map.lookup nodeEid mapping
  in do
  case mbNode of
    Just aNode -> do
